...
def opensearch_credential(**kwargs):
    """
    Fetch raw credentials for opensearch.
    """
    provider = CredentialFactory.get_provider(mode="airflow", conn_id="opensearch")
    config = provider.get_credentials()
    return config
...
with DAG(
    ...
    opensearch_credential_task = PythonOperator(
    task_id='opensearch_credential_task',
    python_callable=opensearch_credential,
    )
    ...
    [credentials_task, es_credential_task, opensearch_credential_task] >> extraction_task
